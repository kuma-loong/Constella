...
SQLite 历史库默认关闭。设置 `DB_PATH=run/constella.db` 后，Rust manager 会写入节点、GPU、任务和历史查询所需数据。`scripts/cluster/*.sh` 调用 Rust `constella cluster ...`，向远端节点同步 release binary 并启动 Rust agent。`scripts/maintenance/db.sh` 调用 Rust `constella db maintain`，用于关闭过期任务 session、聚合 rollup 并清理过期 raw/rollup 数据。
