...
Highres 环形缓冲为每张 GPU 增加 7 个 `float32` 指标数组和有效位掩码。按 8 GPU、1 秒采样、保留 2 小时估算，新增内存约 1.54 MiB；按 0.5 秒容量预留约 3.1 MiB。

接口按 Node、GPU、指标和时间范围批量查询，并支持 `max_points` 与 `summary_only`。默认仅请求已展开指标，每条曲线限制约 1500 个绘图点，并取消 Node/GPU 切换后迟到的旧请求。
