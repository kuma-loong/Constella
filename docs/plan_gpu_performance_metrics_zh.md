...
图表悬停时使用共享的垂直时间线，并在图表下方固定检查栏展示采样时间及各 GPU 的精确值，不遮挡曲线；拖拽选择仍用于计算选定时间段的均值、峰值、P95 和覆盖率，两种交互互不冲突。
...
分类和单张图表都允许折叠，折叠状态保存在浏览器本地。图表固定使用单列完整宽度，避免多 GPU 曲线在窄图中挤压。折叠图表不创建或更新绘图实例。

所有展开图表共享时间轴、垂直时间线和选中区间，背景网格按绘图区固定等分，不随 Live 时间窗口向左滚动。在任意图表框选时间后，统一计算各 GPU 的平均值、峰值、P95、样本数和覆盖率。统计值基于服务端原始数据计算，绘图数据允许降采样；长时间范围采用保留峰值的 min/max bucket 策略。缺失样本在曲线上显示为空洞。

绘图层先对每条曲线的连续有效样本执行权重为 `[1, 2, 3, 2, 1] / 9`
的 5 点平滑，再使用样条曲线连接。平滑不跨越缺失区间；悬停精确值、区间统计
和后端数据继续使用未平滑样本。
...
默认仅请求已展开指标；每条 GPU×指标曲线独立使用 1,000 点预算，预算不随同时
展示的 GPU 或指标数量缩减。5/15 分钟 Live 窗口每 2 秒刷新，1 小时每 5 秒、
2 小时每 10 秒刷新，并取消 Node/GPU 切换后迟到的旧请求。统计值始终基于原始
点计算，不受绘图预算和平滑影响。
